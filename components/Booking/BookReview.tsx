...
type StepProps = {
    ...
    const data = [flights]
    console.log('Flight data booking:', data);
    return (
        <>
    {data && data.length > 0 &&  data?.map((flight,index)=>( 
        <div  className=' w-full p-4 my-8' key={index}>
            <div  className=' grid gap-4'>
                <div className=' flex items-center gap-2 my-2'>
                    <div className=' flex items-center'>
                        <h2 className=' text-sm font-semibold'>{flight[0].displayData.source.airport.cityName}</h2>
                        <ArrowRight className='w-4 h-4 text-foreground mx-1'/>
                        <h2 className='text-sm font-semibold'>{flight[0].displayData.destination.airport.cityName}</h2>
                    </div>
                    <p className='  text-sm font-light'>{flight[0].displayData.source.depTime.slice(11, 16)}</p>
                ...
                <div className=' flex items-center gap-4 mb-5'> 
                    <div>
                        ...
                        <p className='w-20 text-xs text-foreground'>{flight[0].displayData.airlines[0].airlineCode}{flight[0].displayData.airlines[0].airlineName}{flight[0].displayData.airlines[0].flightNumber}</p>
                    </div>
                    <div className='grid gap-4'>
                        <div className=' flex items-center gap-2'>
                            <p className=' font-semibold text-2xl text-black'>{flight[0].displayData.source.depTime.slice(14, 16)}</p>
                            <p className=' text-lg text-foreground '>{flight[0].displayData.source.airport.cityCode}</p>
                            <p className=' text-xs text-foreground'>{flight[0].displayData.source.airport.airportName}, {flight[0].displayData.source.airport.terminal} </p>
                        </div>
                        <Clock5 className=' w-4 h-5 text-foreground'/>
                        <div className=' flex items-center gap-2'>
                            <p className=' font-semibold text-2xl text-black'>{flight[0].displayData.destination.arrTime.slice(14, 16)}</p>
                            <p className=' text-lg text-foreground '>{flight[0].displayData.destination.airport.cityCode}</p>
                            <p className=' text-xs text-foreground'>{flight[0].displayData.destination.airport.airportName}, {flight[0].displayData.destination.airport.terminal}</p>
                        </div>
                    </div>
                </div>
                <div className=' grid items-center'>
                    <div className="border-b border-gray-300 bg-blue-100 p-4">
                        <div className=' flex items-center gap-2'>
                            <h1 className=' text-sm font-bold'>{flight[0].displayData.source.airport.cityCode}</h1>
                            <ArrowRight className=' h-5 w-5'/>
                            <h1 className='text-sm font-bold'>{flight[0].displayData.destination.airport.cityCode}: Standard Fare</h1> 
                        </div>
                    </div>
